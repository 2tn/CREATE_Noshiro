...
uint8_t SPI0_Readbyte(uint8_t address) {
	uint8_t data[1];
...
void ReadRegs(uint8_t ReadAddr, uint8_t *ReadBuf, uint16_t Bytes) {
	uint16_t i = 0;
	uint8_t temp_val;
	LPC_SSP0->DR = ReadAddr | READ_FLAG;
	while (LPC_SSP0->SR & 0x10)
		;
	temp_val = LPC_SSP0->DR;
	for (i = 0; i < Bytes; i++) {
		LPC_SSP0->DR = 0x00;
		while (LPC_SSP0->SR & 0x10)
			;
		ReadBuf[i] = LPC_SSP0->DR;
	}
}
...
uint8_t WriteReg(uint8_t WriteAddr, uint8_t WriteData) {
	...
	LPC_SSP0->DR = WriteAddr;
	while (LPC_SSP0->SR & 0x10)
		;
	temp_val = LPC_SSP0->DR;

	LPC_SSP0->DR = WriteData;
	while (LPC_SSP0->SR & 0x10)
		;
	temp_val = LPC_SSP0->DR;
	volatile uint16_t i;
...
void Servo_initialize() {
	...
	LPC_TMR16B0->PR = 480 - 1;	//100kHz
	LPC_TMR16B0->MCR |= (1 << 0);
	LPC_TMR16B0->MCR |= (1 << 3);
	LPC_TMR16B0->MCR |= (1 << 4);
	LPC_TMR16B0->MR0 = 80; 		//5～24まで
	LPC_TMR16B0->MR1 = 2000;
	LPC_TMR16B0->TCR = 1;
